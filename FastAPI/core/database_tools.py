...
def parts_summary(articles_path: str, parts_path: str) -> list:
    ...
    try:
        ...
        mode_str = lambda s: (m.iloc[0] if not (m := s.dropna().mode()).empty else "")
        mode_int = lambda s: (int(m.iloc[0]) if not (m := s.dropna().mode()).empty else 0)
        ...
        art_summary = (
            ...
            .agg(
                ...
                mostCommonCountryOfOrigin=("countryOfOrigin", mode_str),
            )
            .reset_index()
        )
        ...
        parts_summary_df = (
            ...
            .agg(
                partDescription=("partDescription", mode_str),
                quantity=("quantity", mode_int),
            )
            .reset_index()
        )
...
def top_5_parts_by_price(articles_path: str, parts_path: str) -> list:
    """Top 5 productIds by bottom-quartile average price (<= Q1), with quantity and numArticles."""
    try:
        # --- Load & clean ---
        articles = pd.read_csv(articles_path)
        parts    = pd.read_csv(parts_path)
...
@tool
def top_5_part_distribution_by_country(articles_path: str, parts_path: str = None) -> list:
    """
    Returns the top 5 countries by number of unique parts (unique articleNo).
...
@tool
def bottom_quartile_average_price(articles_path: str, parts_path: str) -> list:
    """
    Returns bottom quartile average price per productId using the bottom-quantile cut-off method,
    with partDescription, quantity, taxable, and line item total (excl. VAT).
    """
    try:
        # --- Load & light cleanup ---
        articles = pd.read_csv(articles_path, dtype={"articleNo": str})
...
def total_component_price(articles_path: str, parts_path: str, vat_rate: float) -> dict:
    ...
    prices per productId, charging VAT only on taxable parts.
    """
    try:
...
def top_5_suppliers_by_articles(articles_path: str) -> list:
    """Returns the top 5 suppliers by the number of unique articles in articles.csv."""
    try:
...
def calculator(expression: str) -> str:
    """Safely evaluates arithmetic like `sum([47.02,130.76])`, `round(240.64/458.58*100,2)`, `mean([..])`, `sqrt(2)`.
    Allowed: + - * / ** % //, parentheses, lists/tuples, sum/min/max/round/sqrt/log/log10/mean, pi, e."""
    try:
